...
This chapter describes the unified experimental protocol used across the three phases of the thesis. Section 3.1 describes the dataset and splits. Section 3.2 covers preprocessing and augmentation. Section 3.3 describes the training pipeline shared by all architectures. Section 3.4 lists the evaluation metrics. Section 3.5 introduces the three classical-baseline classifiers. Section 3.6 describes the conformal prediction protocol, Section 3.7 the Monte Carlo Dropout setup, and Section 3.8 the OOD detection protocol. Section 3.9 describes K-fold cross-validation. Section 3.10 details the multi-class extension. Section 3.11 describes the implementation, code organisation, deployment artefacts (Streamlit application), and reproducibility conventions.

## 3.1 Dataset and Splits

### 3.1.1 APTOS 2019

All experiments use the APTOS 2019 Blindness Detection dataset, comprising 3,662 retinal fundus images at 224×224 pixels, each labelled by a clinician on the 0–4 severity scale. The dataset is loaded once at the start of each experiment and cached as a single pandas DataFrame on disk to ensure that every experiment sees the same image order and identical pixel values. The class distribution is heavily skewed, dominated by the No DR class, as shown in Figure 3.1.

![**Figure 3.1.** Class distribution of the APTOS 2019 dataset across the five severity levels (0 = No DR, 1 = Mild, 2 = Moderate, 3 = Severe, 4 = PDR). The dataset is heavily imbalanced, motivating the use of class-balanced loss weighting in training.](../../../results/input_data_histogram.png)
...
These are applied **only to the training set**. The validation and test generators apply the same architecture-specific preprocessing but no augmentation, so that all metrics are computed on un-perturbed images. To enforce this cleanly the pipeline exposes `build_train_datagen` and `build_eval_datagen` as separate functions in `lib/helpers.py`, ensuring that the test set is never augmented.
...
- **Bootstrap 95% confidence intervals** [38] on test accuracy, computed with 1,000 resamples and seed 42.
- **Pairwise McNemar tests** [39] between every model pair, with a continuity correction. p < 0.05 indicates a statistically significant difference in error rates.
- **Cohen's kappa** [40] between pairs of classifiers, as a chance-corrected agreement measure.

## 3.5 Classical Baseline Classifiers

To anchor the comparison of deep architectures against simpler methods, three classical machine-learning classifiers are also evaluated: a Decision Tree, a Random Forest, and a Support Vector Machine. Rather than training them on raw 224×224×3 pixels — which would yield 150,528-dimensional feature vectors and would be both computationally infeasible and statistically poor — the classical models are trained on **deep features extracted from a frozen DenseNet121 backbone with ImageNet pretrained weights**. The base network is loaded with `include_top=False, pooling='avg'`, producing a 1024-dimensional feature vector per input image. This convention follows the standard "deep-features + shallow classifier" approach used widely in transfer-learning practice.

The three classifiers, all from `scikit-learn`, are configured as follows:

- **Decision Tree** (`DecisionTreeClassifier`): default Gini split criterion with `class_weight='balanced'`, `random_state=123`.
- **Random Forest** (`RandomForestClassifier`): 300 trees, `class_weight='balanced'`, `random_state=123`, `n_jobs=-1`.
- **Support Vector Machine** (`SVC`): RBF kernel, `probability=True`, `class_weight='balanced'`, `random_state=123`.

The classical models use a stratified 80/20 split, since they do not need a separate validation set for early stopping or learning-rate scheduling. Test accuracy and confusion matrices are reported alongside ROC and precision-recall curves. The implementation lives in [`scripts/ML.py`](scripts/ML.py).

## 3.6 Conformal Prediction Protocol

### 3.6.1 Calibration Set
...
## 3.7 Monte Carlo Dropout Protocol

### 3.7.1 Architecture Variants
...
## 3.10 Multi-Class Extension

### 3.10.1 Label Reformulation

Phase 3 reformulates the task using the original 5-class severity labels (0 to 4) instead of the binary collapse used in Phases 1–2. The same 70/15/15 split is used, but stratified on the 5-class label so that each split contains the appropriate proportion of each severity level. The class distribution of the test set is heavily imbalanced: No DR 271, Mild 56, Moderate 150, Severe 29, PDR 44.

### 3.10.2 Architecture Heads
...
### 3.10.4 Multi-Class Conformal Prediction
...
### 3.10.5 Multi-Class Calibration
...
### 3.11.5 Streamlit Decision-Support Application

A complete deployment artefact accompanies the thesis: an interactive Streamlit application ([`app.py`](app.py), with helper modules in [`lib/`](lib/)) that exposes all four uncertainty signals to a non-technical end user. The application is the practical demonstration that the uncertainty machinery developed in this thesis can be packaged for clinical workflow integration. Its main components are:

- **Single-image and batch inference.** The user can upload a single fundus photograph or a folder of up to several hundred images. Each image is preprocessed with the architecture-specific pipeline of Section 3.2 and routed to the user-selected primary model.
- **Calibrated probabilities and conformal sets.** For every image, the application reports the calibrated softmax probability for each of the five severity classes, together with a conformal prediction set at user-selected α (90% or 95% coverage). When the conformal set has size ≥ 3, the UI surfaces a "refer to clinician" recommendation.
- **Ensemble disagreement.** The user can optionally select additional comparison models; the application then displays per-image disagreement (standard deviation across the selected ensemble members) as an additional uncertainty signal.
- **Image-quality heuristics.** A lightweight quality module ([`lib/quality.py`](lib/quality.py)) flags inputs whose foreground brightness, contrast, or focus fall outside thresholds calibrated on retinal fundus photography, gating downstream inference behind a quality check.
- **PDF report export.** A dedicated report module ([`lib/report.py`](lib/report.py)) produces a multi-page PDF summarising the batch results — a one-page cover with batch-level metadata followed by per-image detail pages — for clinical record-keeping and sharing.
- **Multilingual interface.** All user-facing strings are externalised to JSON dictionaries ([`translations/en.json`](translations/en.json), [`translations/sq.json`](translations/sq.json)), with the language selector exposed in the sidebar. This is a deliberate accessibility choice for the bilingual deployment context this thesis was developed in.
- **Inference history.** A session-local history panel records the most recent inputs and predictions for quick comparison without re-uploading.

The application is intentionally read-only at the model level: it loads pre-trained `.keras` checkpoints and pre-computed conformal thresholds (`app_conformal_thresholds.json`) and never retrains anything at user request. This ensures that the statistical guarantees established by the offline evaluation pipeline carry through to the deployed tool unchanged. Representative screenshots of the running application — the binary prediction view, the multi-model comparison view, and the 5-stage conformal grading view — are reproduced in Shtojca G.
